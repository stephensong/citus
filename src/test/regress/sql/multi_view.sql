...
-- These are the settings that lineitem_view_test and orders_view_test tables
-- are created with.
SET citus.shard_count to 4;
SET citus.shard_replication_factor to 1;

SELECT count(*) FROM lineitem_view_test;

SELECT count(*) FROM orders_view_test;

-- create a view for priority orders
CREATE VIEW priority_orders AS SELECT * FROM orders_view_test WHERE o_orderpriority < '3-MEDIUM';

-- aggregate pushdown
SELECT o_orderpriority, count(*)  FROM priority_orders GROUP BY 1 ORDER BY 2, 1;

SELECT o_orderpriority, count(*) FROM orders_view_test  WHERE o_orderpriority < '3-MEDIUM' GROUP BY 1 ORDER BY 2,1;
...
-- limit
SELECT o_orderkey, o_totalprice from orders_view_test order by 2 desc, 1 asc limit 5 ;

SELECT o_orderkey, o_totalprice from priority_orders order by 2 desc, 1 asc limit 1 ;

CREATE VIEW priority_lineitem AS SELECT li.* FROM lineitem_view_test li JOIN priority_orders ON (l_orderkey = o_orderkey);

SELECT l_orderkey, count(*) FROM priority_lineitem GROUP BY 1 ORDER BY 2 DESC, 1 LIMIT 5;

CREATE VIEW air_shipped_lineitems AS SELECT * FROM lineitem_view_test WHERE l_shipmode = 'AIR';

-- join between view and table
SELECT count(*) FROM orders_view_test join air_shipped_lineitems ON (o_orderkey = l_orderkey);

-- join between views
SELECT count(*) FROM priority_orders join air_shipped_lineitems ON (o_orderkey = l_orderkey);

-- count distinct on partition column is not supported, must be supported
...
-- however, this works
SELECT count(*) FROM priority_orders left join lineitem_view_test ON (o_orderkey = l_orderkey) WHERE l_shipmode ='AIR';

-- view at the inner side of is not supported
SELECT count(*) FROM priority_orders right join lineitem_view_test ON (o_orderkey = l_orderkey) WHERE l_shipmode ='AIR';

-- but view at the outer side is. This is essentially the same as a left join with arguments reversed.
SELECT count(*) FROM lineitem_view_test right join priority_orders ON (o_orderkey = l_orderkey) WHERE l_shipmode ='AIR';
...
CREATE TABLE temp_lineitem(LIKE lineitem_view_test);
SELECT create_distributed_table('temp_lineitem', 'l_orderkey');
...
SELECT l_suppkey, count(*) FROM
	(SELECT l_suppkey, l_shipdate, count(*)
		FROM lineitem_view_test WHERE l_shipmode = 'AIR' GROUP BY l_suppkey, l_shipdate) supps
	GROUP BY l_suppkey ORDER BY 2 DESC, 1 LIMIT 5;

-- when a view is replaced by actual query it still fails
SELECT l_suppkey, count(*) FROM
	(SELECT l_suppkey, l_shipdate, count(*)
		FROM (SELECT * FROM lineitem_view_test WHERE l_shipmode = 'AIR') asi
...
CREATE VIEW lineitems_by_shipping_method AS
	SELECT l_shipmode, count(*) as cnt FROM lineitem_view_test GROUP BY 1;
...
CREATE VIEW lineitems_by_orderkey AS
	SELECT l_orderkey, count(*) FROM lineitem_view_test GROUP BY 1;
...
DROP TABLE lineitem_view_test CASCADE;
DROP TABLE orders_view_test CASCADE;
